  ...
  function renderData(data) {
    ...
    data.forEach((item) => {
      ...
      dataHTML += `
          <div class="konten-items rounded-2 border position-relative d-flex justify-content-center align-items-center">
            <img class="position-absolute top-0 start-0" width="30px" id="${item.nama_produk}" onclick="btnName('${item.nama_produk}')" src="../assets/icon/plus-bandingkan.png" alt="${item.nama_produk}">      
            ${imgHTML}
    ...
    let apple = document.getElementById('Apple Vision Pro');
    let pico = document.getElementById('Pico 4')
